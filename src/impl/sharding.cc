...
ReplicatedSpecValue ReplicatedSpecValue::make(bool immutable) {
  auto n = make_object<ReplicatedSpecValueObj>();
  n->immutable = immutable;
  return ReplicatedSpecValue(n);
}

ShardSpecValue ShardSpecValue::make(bool immutable, Array<Integer> ranks, Array<Integer> phy_shape,
                          Array<Integer> replicas) {
  auto ndim = phy_shape.size();
  CHECK_EQ(ndim, replicas.size());
  auto n = make_object<ShardSpecValueObj>();
  ...
  return ShardSpecValue(n);
}

TupleSpecValue TupleSpecValue::make(bool immutable, Array<BaseSpecValue> tuple_elem) {
  auto n = make_object<TupleSpecValueObj>();
  n->immutable = immutable;
  n->tuple_elem = tuple_elem;
  return TupleSpecValue(n);
}

Attrs ShardOpCallAttrs::make(BaseSpecValue shard_in, BaseSpecValue shard_out) {
...
void Reshard_R2S(const CallValues& call) {
  ...
  auto spec = Downcast<ShardSpecValue>(args->spec);
...
RAF_REGISTER_GLOBAL("raf.sharding._make.ReplicatedSpecValue").set_body_typed(ReplicatedSpecValue::make);
RAF_REGISTER_GLOBAL("raf.sharding._make.ShardSpecValue").set_body_typed(ShardSpecValue::make);
RAF_REGISTER_GLOBAL("raf.sharding._make.TupleSpecValue").set_body_typed(TupleSpecValue::make);
RAF_REGISTER_GLOBAL("raf.sharding._make.ShardOpCallAttrs").set_body_typed(ShardOpCallAttrs::make);

RAF_REGISTER_OBJECT_NO_REFLECT(BaseSpecValueObj);
RAF_REGISTER_OBJECT_REFLECT(ReplicatedSpecValueObj);
RAF_REGISTER_OBJECT_REFLECT(ShardSpecValueObj);
RAF_REGISTER_OBJECT_REFLECT(TupleSpecValueObj);
...
void PrintAllocTable(const ObjectRef& ref, ReprPrinter* p) {
  /*size_t dev_idx = 0;
  const auto obj = Downcast<ShardSpecValue>(ref);
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<ReplicatedSpecValueObj>([](const ObjectRef& ref, ReprPrinter* p) {
      auto r = Downcast<ReplicatedSpecValue>(ref);
      p->stream << "ReplicatedSpecValue" << (r->immutable ? "(Immut)" : "");
    });

TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<ShardSpecValueObj>([](const ObjectRef& ref, ReprPrinter* p) {
      auto r = Downcast<ShardSpecValue>(ref);
      auto ndim = r->logic_shape.size();
      p->stream << "ShardSpecValue(" << (r->immutable ? "Immut " : "") << "[";
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<TupleSpecValueObj>([](const ObjectRef& ref, ReprPrinter* p) {
      auto r = Downcast<TupleSpecValue>(ref);
      p->stream << "TupleSpecValue" << (r->immutable ? "(Immut)" : "");
    });
